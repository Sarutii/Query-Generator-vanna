...
class MyVanna(ChromaDB_VectorStore, Ollama):
    ...
    def get_related_ddl(self, question: str, n_results: int = 5, threshold: float = 0.2) -> list:
        ...
        try:
            ...
            question_embedding = embedding_func([question])[0]
            ...
            for collection in collections:
                try:
                    ...
                    results = collection.query(
                        ...
                        n_results=n_results * 2,  # Get extra results to filter
                    ...
                    if results and 'documents' in results and 'distances' in results:
                        for doc, distance in zip(results['documents'][0], results['distances'][0]):
                            ...
                            content = doc.upper()
                            # Check if it's a DDL statement and meets threshold
                            if (any(keyword in content for keyword in ['CREATE TABLE', 'CREATE VIEW', 'ALTER TABLE']) and
                                similarity >= threshold):
                                ddl_results.append((doc, similarity))
                                # Log the retrieved DDL and its similarity score
                                logging.info(f"Retrieved DDL: {doc[:100]}... with similarity score: {similarity:.3f}")
                except Exception as e:
                    logging.debug(f"ChromaDB DDL query failed for collection {collection.name}: {e}")
                    continue

            # Sort by similarity score (descending) and take top N
            ddl_results = sorted(ddl_results, key=lambda x: x[1], reverse=True)[:n_results]
            return [ddl for ddl, _ in ddl_results]  # Return only the DDLs

        except Exception as e:
            logging.warning(f"Could not retrieve DDL context: {e}")
            return []

    def get_related_documentation(self, question: str, n_results: int = 3, threshold: float = 0.2) -> list:
                ...
                try:
                    ...
                    if results and 'documents' in results and 'distances' in results:
                        for doc, distance in zip(results['documents'][0], results['distances'][0]):
                            ...
                            similarity = 1 - distance
                            content = doc.lower()
                            # Check if it's documentation-like and meets threshold
                            if (any(keyword in content for keyword in ['contains', 'table', 'column', 'field', 'stores', 'represents']) and
                                not any(keyword in content.upper() for keyword in ['SELECT', 'CREATE TABLE', 'INSERT']) and
                                similarity >= threshold):
                                doc_results.append((doc, similarity))
                                # Log the retrieved documentation and its similarity score
                                logging.info(f"Retrieved Documentation: {doc[:100]}... with similarity score: {similarity:.3f}")
                except Exception as e:
                    logging.debug(f"ChromaDB doc query failed for collection {collection.name}: {e}")
                    continue

            # Sort by similarity score (descending) and take top N
            doc_results = sorted(doc_results, key=lambda x: x[1], reverse=True)[:n_results]
            return [doc for doc, _ in doc_results]  # Return only the documents
