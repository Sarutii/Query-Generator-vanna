...
# Load allowed table names from CSV
schema_df = pd.read_csv(r"C:\Users\ahmed\Desktop\Projects\Query Generator\Vanna_app\Query-Generator-vanna\Table_comments_final.csv")
allowed_table_names = schema_df['table_name'].dropna().str.upper().tolist()

class MyVanna(ChromaDB_VectorStore, Ollama):
    def __init__(self, config=None, allowed_tables=None):
        config = config or {}
    ...
    def _extract_table_names(self, sql: str):
        return set(re.findall(r'\bFROM\s+([A-Z_]+)|\bJOIN\s+([A-Z_]+)', sql, re.IGNORECASE))
    ...
    def generate_sql(self, question: str) -> str:
        ...
        while attempt < max_attempts:
            sql = super().generate_sql(full_prompt)
            found_tables = set(filter(None, [t.upper() for pair in self._extract_table_names(sql) for t in pair]))
            if found_tables.issubset(self.allowed_tables):
                return sql
            attempt += 1
        raise ValueError("Unable to generate SQL using only allowed table names after several attempts.")
...
# Initialize Vanna with allowed tables
vn = MyVanna(allowed_tables=allowed_table_names)

# Add table names to the prompt prefix
tables_str = ", ".join(allowed_table_names)
