...
class MyVanna(ChromaDB_VectorStore, Ollama):
    ...
    def generate_sql(self, question: str) -> str:
        # Use RAG (retrieve relevant documents from vector store)
        context = self.query(question=question)
...
try:
    ...
    print(f"Loaded {len(table_names)} table names.")
except Exception as e:
    print(f"Warning: Could not load table names from CSV: {e}")

tables_str = ", ".join(table_names) if table_names else "the tables you were trained on"

vn = MyVanna()

# Minimal prefix (no full training content)
vn.set_prompt_prefix(f"""
You are an expert SQL assistant writing SQL queries for an Oracle Database.
Follow these strict Oracle SQL rules:
- Use only the following valid table names: {tables_str}.
- Do NOT fabricate table names or columns.
- Use Oracle syntax like ROWNUM, SYSDATE, TO_DATE('YYYY-MM-DD', 'YYYY-MM-DD'), and DUAL.
- Avoid MySQL/PostgreSQL-specific syntax.
Return only clean, executable SQL.
""")
...
def ask_sql():
    ...
    try:
        for _ in range(3):  # Try up to 3 times to generate valid SQL
            sql = vn.generate_sql(question=question)
            if any(tbl in sql for tbl in table_names):
                break
        else:
