...
def extract_Column_info(sql_file_path=None, text = None):
    """
    Extract column names and their comments from text following the pattern:
    COMMENT ON COLUMN column_name IS 'comment'
    
    Args:
        text (str): Input text containing column definitions
        
    Returns:
        pd.DataFrame: DataFrame with columns 'column_name' and 'comment'
    """
    ...
    for line in lines:
        if "COLUMN" in line:
            try:
                # Split by "column" to get parts after each occurrence
                parts = re.split(r'COLUMN\s+', line)
                
                # Skip the first part (before the first "COLUMN")
                for part in parts[1:]:
                    # Extract COLUMN name (first word after "COLUMN")
...
# Example usage
sample_text = """COMMENT ON COLUMN  ACCOUNT_GROUPING IS ' ãÌãæÚÉ ÇáÍÓÇÈÇÊ  ; '"""

# Process the text and get the DataFrame
result_df = extract_Column_info(sql_file_path= r"C:\Users\ahmed\Desktop\Projects\Query Generator\Vanna_app\Table_Column_Comment.sql")

# Display the result
print(result_df)
#save to csv
#but encode every column on its own encoding
result_df.to_csv(r"C:\Users\ahmed\Desktop\Projects\Query Generator\Vanna_app\COLUMN_Comment.csv", index=False)
...
COMMENT ON COLUMN  USER_ACCOUNTS IS ' ÍÓÇÈÇÊ ÇáãÓÊÎÏãíä ; '
COMMENT ON COLUMN  TRANSACTION_LOG IS ' ÓÌá ÇáãÚÇãáÇÊ ; '\"""
